type ActionsType = ReturnType<typeof setButton>
...
export const calcReducer = (state: InitialStateType = initialState, action: ActionsType): InitialStateType => {
    switch (action.type) {
        case "SET-BUTTON":
            return {
                ...state, num: state.num.concat(action.num)
            }
        case "SET-RESULT":
            try {
                return {
                    ...state, result: eval(state.num).toString()
                }
            } catch (error) {
                return {
                    ...state, result: "error"
